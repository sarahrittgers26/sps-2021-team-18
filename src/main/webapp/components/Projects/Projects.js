...
const Projects = ({ history }) => {
    ...
    socket.onmessage = (response) => {
        ...
        switch (msg.type) {
            case ACTION.PING_USER:
                ...
                } else if (ping_type === "delete") {
                    let notiReceived = {
                        collaboratorName: collaboratorName,
                        collaborator: collaboratorId,
                        projectid: info_arr[4],
                        isNewProject: false,
                        projectTitle: info_arr[5],
                        type: ping_type,
                        proj: { collaboratorId: collaboratorId, projectid: info_arr[4] },
                        isDeletion: true,
                        collaboratorAvatar: collabAvatar
                    };
                    setNotifications([...notifications, notiReceived]);
    ...
    const continueProject = (projectid, title, collaborator, collaboratorName,
        ...
        if (isActive) {
            ...
            dispatch(selectCollab({
                name: collaboratorName,
                username: collaborator,
                avatar: avatar
            }));
            dispatch(chooseProject({
                projectid: projectid,
    ...
    const deleteProjectDialog = (projectid, title, collaborator, collaboratorName,
        html, css, js) => {
        let avatar = "0";
        let isActive = true;
        for (var i = 0; i < contacts.length; i++) {
        ...
        if (isActive) {
            ...
            setConnectionAlert(
                `This will ask ${collaboratorName} to confirm DELETING ${title}!`
            );
        } else {
            setOpenAlertDialog(true);
            setAlertWarning(
                `${collaboratorName} is currently offline. You can only delete 
	        projects when you are both online.`
            );
        }
    }
    ...
    const getProjectComponents = () => {
        let projects = displayedProjects.map((project) => ( 
            ...
            downloadProject = {
                () => downloadProject(project.title, project.html, project.css, project.js) }
            continueProject = {
                () => continueProject(
    ...
    const acceptCallBack = (not_type, info) => {
        if (not_type === "create") {
            ...
            let data = `yes=${user.avatar}=${user.username}=${user.name}`;
            let msg = JSON.stringify({
                id: info.collaborator,
                type: ACTION.REC_CONTINUE_PING,
                data: data
            })
            socket.send(msg);
            dispatch(chooseProject({
                ...
                js: info.js,
                title: info.title,
                collaboratorAvatar: info.avatar
            }));
            dispatch(updateEdit(true));
        } else if (not_type === "delete") {
            let msg = JSON.stringify({
                id: info.collaboratorId,
                type: ACTION.REC_DELETE_PING,
                data: `yes=${info.projectid}`
            })
            socket.send(msg);
            dispatch(loadProjects(user.username));
        }
    ...
    const declineCallBack = (collabId, ping_type) => {
        if (ping_type === "create") {
            ...
            socket.send(msg);
        } else if (ping_type === "continue") {
            let msg = JSON.stringify({ id: collabId, type: ACTION.REC_CONTINUE_PING, data: "no" })
            socket.send(msg);
        } else if (ping_type === "delete") {
            let msg = JSON.stringify({ id: collabId, type: ACTION.REC_DELETE_PING, data: "no" })
            socket.send(msg);
        }
    ...
    return ( <
        ...
        {
            !loadingProjects && allProjects.length === 0 && ( <div className = "card Projects_no_result" >
                <span className = "no_result_summary" >
                No projects </span> <span className = "no_result_message" >
                You have not collaborated on any projects with anyone.Please select any active user to start collaborating on your first project. </span> 
                </div>
            )
        }

        {
            !loadingProjects && allProjects.length !== 0 && displayedProjects.length === 0 && ( <
                ...
                <span className = "no_result_summary" >
                No results found </span> <span className = "no_result_message" >
                You do not have any project that matches the given name. </span> </div>
            )
        }

        {
            !loadingProjects && displayedProjects.length > 0 && ( <div className = "Projects_recent" > { getProjectComponents() } </div>
            )
        } </div>

        <div className = "Projects_sidebar" >
        <ConnectedUsers activeUsers = { activeUsers }
        contacts = { contacts }
...

        {
            openConnectionDialog && ( <
                ConnectionDialog collaboratorName = { currentConnection.name }
        ...
        {
            openAlertDialog && ( <
                AlertDialog isOpen = { openAlertDialog }
        ...
        {
            openProfileDialog && ( <
                ProfileDialog isOpen = { openProfileDialog }
                name = { user.name }
