...
public class DeselectProjectServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		if (user1.equals(username)) {
			project = Entity.newBuilder(datastore.get(thisProject)).set("user1Selected", false).build();
			datastore.update(project);
		} else {
			project = Entity.newBuilder(datastore.get(thisProject)).set("user2Selected", false).build();
			datastore.update(project);
		}
	}
}
